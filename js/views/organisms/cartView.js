...
import { Button, Div, Image, Li, Ul } from "../atoms/index.js"
...
export const cartListView = (data = []) => {
    ...
    data.forEach(item => {
        ...
        price.innerText = PriceToDkk(item.product.price)
        li.append(price)

        const action = Div('text-right w-[10%]')
        const delBtn = Button('Fjern', 'button', 'text-red-800')
...
export const cartHeaderView = arrColumns => {
...
export const cartTotalView = totalPrice => {
    ...
    totalCol.innerText = PriceToDkk(totalPrice)
